...
while running:      # main loop of program. will repeat until the window is closed
    ...
    for e in pygame.event.get():    # get each event in the event queue... 
        ...
        if e.type == pygame.KEYDOWN:
            ...
            if pygame.key.name(e.key) == 'up':
                y -= 5
            elif pygame.key.name(e.key) == 'down':
                y += 5
            elif pygame.key.name(e.key) == 'right':
                x += 5
            elif pygame.key.name(e.key) == 'left':
                x -= 5
    ...
    screen.blit(text_surface, (x, y))   # display the text on the screen
